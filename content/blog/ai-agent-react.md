...
关键点：推理和行动是交替进行、相互促进的。推理指导行动，行动的结果又反过来修正和推进推理。最著名的实现就是ReAct论文提出的提示词模式

## Agent Loop 是什么？
这是智能体持续运行的基本工作流程或生命周期。它是一个周而复始的循环，直到任务完成或终止。一个典型的Agent Loop包含以下步骤：
感知：获取用户输入或环境反馈。
规划/决策：决定下一步要做什么。（这里就包含了Reasoning）
行动：执行决策，调用工具或产生响应。（这里就包含了Acting）
反思/学习：评估行动结果，更新内部状态或记忆。
循环：回到第1步，带着新的信息继续。
关键点：Loop强调的是持续性、迭代性和闭环控制。
